...
def extract_feature_vector(image_path, plot=False):
    ...
    # resize
    resized = cv.resize(cropped_image, (300,300), interpolation = cv.INTER_AREA)
...
def main():
    ...
    if args.image:
        ...
        v = extract_feature_vector(args.image, args.visualize)
    ...
    if args.folder:
        ...
        image_files = [ os.path.join(args.folder, file) for file in os.listdir(args.folder) ]

        results = []
        with multiprocessing.Pool(processes=3) as pool:
            for result in tqdm(pool.imap_unordered(extract_feature_vector, image_files), total=len(image_files)):
